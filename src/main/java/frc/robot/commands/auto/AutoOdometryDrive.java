...
public class AutoOdometryDrive extends Command {
  ...
  public AutoOdometryDrive(SwerveDrive swerveDrive, Vector2 relativeTargetPosition, double speed) {
    ...
    this.speed = SillyMath.clamp(speed, DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_MIN_SPEED, DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_MAX_SPEED);
  ...
  public void execute() 
  {
    ...
    if (distanceToFieldSpaceTargetPos <= DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_TARGET_ALLOWED_ERROR)
    {
      madeItToTarget = true;
    ...
    else
    {
      ...
      if (distanceToFieldSpaceTargetPos < DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_SLOWDOWN_DISTANCE)
      {
        adjustedSpeed = speed * (distanceToFieldSpaceTargetPos / DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_SLOWDOWN_DISTANCE);
        if (adjustedSpeed < DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_MIN_SPEED) { adjustedSpeed = DriveConstants.SwerveConstants.AUTO_ODOMETRY_DRIVE_MIN_SPEED; }
      }
